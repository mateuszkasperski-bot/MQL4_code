...
input double Risk_Per_Trade_Percent = 0.5;
input double Daily_Loss_Limit_Percent = 2.0;
...
void OnTick()
{
   ...
   Comment(" "+"\nATR_Size:  "+NormalizeDouble(atrSL,2)+"   ||   M1_ATR5vATR14:  "+NormalizeDouble(M1_ATR5vATR14,2)+"   ||   M15_ATR5vATR14:  "+NormalizeDouble(M15_ATR5vATR14,2)+"   ||   TickSpeed15Sec:  "+NormalizeDouble(speedTick15Sec,2)+"   ||   AvgTickSpace:  "+NormalizeDouble(avgTickSpace,4)+"   ||   DeviationTickSpace:  "+NormalizeDouble(fastTickSpace,4)+"   ||   DDday[%]:  "+NormalizeDouble(GetDailyLossPercent(),2)+"   ||   LossStreak:  "+consecutiveLosses);
...
double CalculateOrderLots(double entryPrice, double stopLossPrice)
{
   if (!Use_Dynamic_Lot)
      return ClampLotSize(Lot_Size);

   double riskMoney = AccountEquity() * (Risk_Per_Trade_Percent / 100.0);
   ...
   double lots = riskMoney / (slDistancePoints * pointValuePerLot);
   return ClampLotSize(lots);
}
...
double GetDailyLossPercent()
{
   ...
   double drawdown = dayStartEquity - AccountEquity();
   if (drawdown <= 0)
      return 0;

   return (drawdown / dayStartEquity) * 100.0;
}
...
void UpdateDailyRiskState()
{
   ...
   if (GetDailyLossPercent() >= Daily_Loss_Limit_Percent)
   {
...
//+------------------------------------------------------------------+
  ...
  bool openShortRC_RSI()
  {
    ...
    bool FREEDOM = false;
      ...
      double lotToUse = CalculateOrderLots(Bid, Ichimoku_SL_value);
...
bool openLongRC_RSI() {
  ...
  bool FREEDOM = false;
    ...
    if(Ichimoku_SL_value > 0.0 && canOpen_Spread()){
    double lotToUse = CalculateOrderLots(Ask, Ichimoku_SL_value);
